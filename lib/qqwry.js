...
Qqwry.prototype.searchIPScopeStream = function(bginIP, endIP, options) {
	...
	var read = function(size) {
		...
		switch (type) {
			case 'csv':
				outstr +=
					i == b_g && outHeader ? format_csv(['begInt', 'endInt', 'begIP', 'endIP', 'Country', 'Area']) : '';
			...
			case 'object':
				...
				if(self.readableFlowing){
					self.push([begInt, endInt, begIP, endIP, Country, Area].join(','));
				}else{
			...
			case 'text':
			default:
				self.push(`${(begIP + ' - ' + endIP).padEnd(35)} ${Country} ${Area}\n`);
				break;
		}
		i += IP_RECORD_LENGTH;
	};
...
function format_csv(val) {
	return val.map(v => csv_decode(String(v))).join(',') + '\n';
}
